...
export class TableManagementComponent implements OnInit {
  ...
  // ⭐ UPDATED: Now points to the Digital Menu route we just created!
  getQrData(tableNumber: number | null): string {
    if (!tableNumber || !this.storeSlug) return '';
    // Uses window.location.origin so it works on localhost now, and your real domain later!
    return `${window.location.origin}/${this.storeSlug}/menu?table=${tableNumber}`;
  }
